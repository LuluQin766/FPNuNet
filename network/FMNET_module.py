...
class PFAE(nn.Module): 
    def __init__(self, dim, in_dim):
        ...
        self.reduce = nn.Conv2d(in_dim, down_dim, kernel_size=1)
        ...
        self.conv6 = nn.Sequential(
            nn.Conv2d(down_dim, down_dim, kernel_size=1),nn.BatchNorm2d(down_dim),  nn.ReLU(True)  #如果batch=1 ，进行batchnorm会有问题
        )
    ...
    def forward(self, x):
        if DEBUG:
            print(f"\n ============= PFAE forward start ============ ")
            print(f" args: self.dim = {self.dim}, self.in_dim = {self.in_dim}")
            print(f" input: x.shape = {x.shape}")

        x = self.down_conv(x)
        x_reduced = self.conv1(x)                     # [B, down_dim, H, W] for skip connection
        conv1 = x_reduced                             # reuse

        if DEBUG:
            print(f" conv1(x): conv1.shape = {conv1.shape}") # [B, down_dim, H, W]

        conv2 = self.conv2(x)
        b, c, h, w = conv2.shape
        
        if DEBUG:
            print(f" conv2(x): conv2.shape = {conv2.shape}")
        ...
        if DEBUG:
            print(f" q_f_2, k_f_2, v_f_2, tepqkv shape: fft.fft2(conv2.float()).shape = {q_f_2.shape}")
        ...
        if DEBUG:
            print(f" attn_f_2 shape: attn_f_2.shape = {attn_f_2.shape}")
        ...
        if DEBUG:
            print(f" out_f_2, out_f_l_2 shape: out_f_2.shape = {out_f_2.shape}, out_f_l_2.shape = {out_f_l_2.shape}")

        out_2 = self.project_out(torch.cat((out_f_2,out_f_l_2),1))
        F_2 = torch.add(out_2, conv2)

        if DEBUG:
            print(f" F_2 shape: F_2.shape = {F_2.shape}")
            print((f" (x_reduced+F_2) shape: (x_reduced+F_2).shape = {(x_reduced+F_2).shape}"))

        conv3 = self.conv3(x_reduced+F_2)
        b, c, h, w = conv3.shape

        if DEBUG:
            print(f" conv3(x_reduced+F_2): conv3.shape = {conv3.shape}")
        ...
        if DEBUG:
            print(f" F_3 shape: F_3.shape = {F_3.shape}")
            print((f" (x_reduced+F_3) shape: (x_reduced+F_3).shape = {(x_reduced+F_3).shape}"))

        conv4 = self.conv4(x_reduced+F_3)
        b, c, h, w = conv4.shape

        if DEBUG:
            print(f" conv4(x+F_3): conv4.shape = {conv4.shape}")
        ...
        if DEBUG:
            print(f" F_4 shape: F_4.shape = {F_4.shape}")
            print((f" (x_reduced+F_4) shape: (x_reduced+F_4).shape = {(x_reduced+F_4).shape}"))

        conv5 = self.conv5(x_reduced+F_4)
        b, c, h, w = conv5.shape
        if DEBUG:
            print(f" conv5(x_reduced+F_4): conv5.shape = {conv5.shape}")
        ...
        if DEBUG:
            print(f" F_5 shape: F_5.shape = {F_5.shape}")

        conv6 = F.adaptive_avg_pool2d(x_reduced, 1)
        conv6 = self.conv6(conv6)
        conv6 = F.interpolate(conv6, size=x.shape[2:], mode='bilinear')

        if DEBUG:
            print(f" conv6(F.adaptive_avg_pool2d(x, 1)): conv6.shape = {conv6.shape}")

        F_out = self.out(self.fuse(torch.cat((conv1, F_2, F_3,F_4,F_5, conv6), 1)))
        if DEBUG:
            print(f" F_out shape: F_out.shape = {F_out.shape}")
            print(" ====== PFAE module forward end ===== \n")
